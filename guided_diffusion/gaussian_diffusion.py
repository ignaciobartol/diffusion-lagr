...
class GaussianDiffusion:
    ...
    def calc_losses_loop(self, model, x_start, model_kwargs=None):
        """
        Compute training losses for all timesteps.

        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :return: a dict with the key "loss" containing a tensor of shape [N x T].
            Some mean or variance settings may also have other keys.
        """
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            ...
            # Compute training losses at the current timestep
            with th.no_grad():
                out = self.training_losses(
                    model,
                    x_start=x_start,
                    t=t_batch,
                    model_kwargs=model_kwargs,
        ...
        output = {key: [] for key in out.keys()}
        for out in outs:
            for key in out:
                output[key].append(out[key])
        for key in output:
            output[key] = th.stack(output[key], dim=1)
